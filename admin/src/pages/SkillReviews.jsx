...
export default function SkillReviews() {
  ...
  async function fetchItems() {
    try {
      ...
      const res = await api.get(
        `/admin/skills-review?status=${statusFilter}&page=${page}&limit=10`
      );
  ...
  const handleAction = async (status) => {
    ...
    if (
      ...
      return toast.error("Feedback is required for corrections or rejection");
    }

    const rawCategoryId = selectedItem.proof?.categoryId;
    const categoryId = rawCategoryId?._id || rawCategoryId;

    if (!categoryId) {
      return toast.error("Category ID is missing for this skill proof");
    }
    ...
    try {
      ...
      await api.put(
        `/admin/skills-review/${selectedItem.providerId}/${categoryId}`,
        {
          status,
      ...
      console.error("Failed to update skill proof", err);
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-xl font-bold text-gray-900">
            Category Skill Reviews
          </h1>
          <p className="text-[11px] text-gray-500 mt-0.5">
            Review provider portfolios and experience to unlock categories
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl shadow-sm border p-12 text-center">
          <div className="text-4xl mb-4">
            <HiClipboardDocumentList className="mx-auto w-10 h-10 text-gray-300" />
          </div>
          <h3 className="text-lg font-medium text-gray-900">
            No skill proofs found
          </h3>
          <p className="text-gray-500">
            There are no skill proofs matching the current filter.
          </p>
        </div>
      ) : (
          ...
          <div className="bg-white rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto p-6">
            ...
            <div className="grid md:grid-cols-2 gap-6 mb-6">
              <div className="bg-gray-50 p-4 rounded-2xl border">
                ...
                <p className="text-lg font-medium text-gray-900">
                  {selectedItem.proof.categoryId?.name}
                </p>
              </div>
              <div className="bg-gray-50 p-4 rounded-2xl border">
                <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedItem.proof.tools &&
                  selectedItem.proof.tools.length > 0 ? (
                    selectedItem.proof.tools.map((tool, i) => (
                      <span
                        key={i}
                        className="px-2 py-1 bg-white border text-gray-700 rounded text-xs"
                      >
                        {tool}
                      </span>
                    ))
            ...
            <div className="mb-6">
              ...
              <div className="bg-gray-50 p-4 rounded-2xl border whitespace-pre-wrap text-gray-800">
                {selectedItem.proof.experienceDescription ||
            ...
            <div className="grid md:grid-cols-2 gap-6 mb-6">
              <div className="bg-gray-50 p-4 rounded-2xl border">
                ...
                {selectedItem.proof.portfolio?.some(
                  (p) => p.type === "before"
                ) && (
                  <div className="mb-4">
                    <span className="text-xs font-semibold text-amber-700 bg-amber-100 px-2 py-1 rounded mb-2 inline-block">
                      Before Service
                    </span>
                    <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                      {Array.from(
                        new Set(
                          selectedItem.proof.portfolio
                        ...
                        <a key={i} href={url} target="_blank" rel="noreferrer">
                          <img
                            src={url}
                            className="w-full h-24 object-cover rounded shadow-sm border hover:opacity-75 transition"
                            alt="Before"
                          />
                        </a>
                      ))}
                ...
                {selectedItem.proof.portfolio?.some((p) => p.type === "after") && (
                  <div className="mb-4">
                    <span className="text-xs font-semibold text-emerald-700 bg-emerald-100 px-2 py-1 rounded mb-2 inline-block">
                      After Service
                    </span>
                    <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                      {Array.from(
                ...
                {selectedItem.proof.portfolio?.some(
                  (p) => !["before", "after"].includes(p.type)
                ) && (
                  <div>
                    <span className="text-xs font-semibold text-gray-600 bg-gray-200 px-2 py-1 rounded mb-2 inline-block">
                      General Work Samples
                    </span>
                    <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                      {selectedItem.proof.portfolio
                        .filter((p) => !["before", "after"].includes(p.type))
                        .map((img, i) => (
                          <a
                            key={i}
                            href={img.url}
                ...
                {(!selectedItem.proof.portfolio ||
                  selectedItem.proof.portfolio.length === 0) && (
            ...
            <div className="mb-8">
              <label className="block text-sm font-bold text-gray-700 mb-2">
                Admin Feedback (Required for corrections/rejections)
              </label>
              <textarea
