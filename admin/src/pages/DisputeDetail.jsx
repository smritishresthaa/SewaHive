...
export default function DisputeDetail() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (!document.hidden) fetchDispute();
    }, 30000);
    ...
    const connect = () => {
      ...
      source.onmessage = (event) => {
        try {
          ...
          const sameDispute =
            String(notification?.disputeId || notification?.metadata?.disputeId || "") === String(id);
          const sameBooking =
            String(notification?.bookingId || notification?.metadata?.bookingId || "") ===
            String(dispute?.bookingId?._id || dispute?.bookingId || "");
          if (
          ...
          console.error("Failed to process dispute detail stream update", err);
        }
      };
      source.onerror = () => {
        source?.close();
  ...
  const heldAmount = Number(
    dispute?.paymentId?.amount ||
      ...
      dispute?.bookingId?.pricing?.agreedPrice ||
      0
  );
  const resolvedRefundAmount = refundAmount ? parseFloat(refundAmount) : 0;
  const providerSettlement = Math.max(0, heldAmount - resolvedRefundAmount);
            ...
            <div className="space-y-6">
              ...
              {dispute.resolutionDetails && (
                <div className="rounded-lg border-l-4 border-green-500 bg-green-50 p-4">
                  ...
                  {Number(dispute.resolutionDetails.providerPayout) > 0 && (
                    <p className="text-sm text-green-800">
                      <strong>Provider Payout:</strong> Rs. {dispute.resolutionDetails.providerPayout}
                    </p>
                  )}
                  {dispute.resolutionDetails.reason && (
                    <p className="mt-2 text-sm text-green-800">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <input
                      ...
                      step="0.01"
                      min="0"
                      max={heldAmount || undefined}
                      className="mt-2 w-full rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-900 focus:border-emerald-500 focus:outline-none"
                    />
                    {heldAmount > 0 && (
                      <p className="mt-2 text-xs text-gray-500">Held amount for this booking: Rs. {heldAmount.toFixed(2)}</p>
                    )}
                  </div>

                  <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
                    <p className="text-sm font-semibold text-amber-900">Settlement Preview</p>
                    <div className="mt-2 space-y-1 text-sm text-amber-800">
                      <p>Client refund: Rs. {resolvedRefundAmount.toFixed(2)}</p>
                      <p>Provider payout: Rs. {providerSettlement.toFixed(2)}</p>
                    </div>
                    {resolutionType === "refund_partial" && (
                      <p className="mt-2 text-xs text-amber-700">
                        This preview helps you simulate a split outcome before resolving the dispute.
                      </p>
                    )}
                  </div>
                </div>
