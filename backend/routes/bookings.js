...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      // Client gets in-app notification only
      await createNotification({
        userId: req.user.id,
        type: "system_message",
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      // Notify client immediately for emergency acceptance
      await createNotification({
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      // 🔔 NOTIFY CLIENT ABOUT REFUND (NEW)
      await createNotification({
        userId: booking.clientId,
...
router.get("/upcoming", authGuard, async (req, res, next) => {
  try {
    ...
    if (bookings.length > 0) {
      ...
      console.log(
        `[BOOKINGS /upcoming] Sample bookings:`,
