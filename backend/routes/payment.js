...
// function applyCreatedAtRangeFilter(filter, range, from, to) {
...
//         return res
//           .status(400)
//           .json({ message: "Cancelled bookings cannot be paid" });
//       }
...
//         return res
//           .status(400)
//           .json({ message: "No payable amount for this booking" });
//       }
...
//     const { transaction_uuid, transaction_code, total_amount, status } =
//       decodedData;
...
//       return res.redirect(
//         `${frontendUrl}/payment/failure?error=payment_not_found`
//       );
//     }
...
//         console.log(
//           "✅ Booking updated:",
//           booking ? booking.status : "NOT FOUND"
//         );
...
//         await wallet.save();
//         console.log(
//           "✅ Wallet updated - Pending balance:",
...
//               isEmergency: booking.type === "emergency",
//             },
//             sendSMS: false,
//           };
...
//         } catch (notifError) {
//           console.error(
//             "⚠️ Failed to send client notification:",
//             notifError.message
//           );
//         }
...
//         } catch (notifError) {
//           console.error(
//             "⚠️ Failed to send provider notification:",
//             notifError.message
//           );
//         }
...
//     const frontendUrl = process.env.FRONTEND_URL || "http://localhost:5173";
//     return res.redirect(
//       `${frontendUrl}/payment/failure?error=verification_failed`
//     );
//   }
// });
...
//       }
//       filter = applyCreatedAtRangeFilter(filter, range, from, to);
...
//           .populate(
//             "bookingId",
//             "serviceTitle serviceId status totalAmount paymentStatus"
//           )
//           .populate("clientId", "profile.name email")
//           .populate("providerId", "profile.name email"),
//         Payment.countDocuments(filter),
...
//         return res
//           .status(400)
//           .json({ message: "Refund already requested for this payment" });
//       }
...
//     return res.redirect(
//       `${frontendUrl}/payment/failure?transaction_uuid=${
...
//         } catch (notifError) {
//           console.error(
//             "Failed to send client payment notification:",
//             notifError.message
//           );
//         }
...
//       if (
//         !isCompletionPendingStatus(booking.status) &&
//         !hasProviderCompletion
//       ) {
//         return res.status(400).json({
...
//         return res
//           .status(400)
//           .json({ message: "Adjusted quote approval is pending" });
//       }
...
//         return res.status(400).json({
//           message: "Booking is under dispute and cannot be completed",
//         });
//       }
...
//             (finalPayment - totalHeldAmount).toFixed(2)
//           ),
//         });
//       }
...
//       booking.pricing.paymentAuditTrail =
//         booking.pricing.paymentAuditTrail || [];
...
//         return res
//           .status(400)
//           .json({ message: "Cannot raise dispute at this stage" });
//       }
...
//           additionalEscrowRequired: Number(
//             (agreedAmount - escrowHeld).toFixed(2)
//           ),
//         });
//       }
...
//         return res
//           .status(404)
//           .json({ message: "Related payment/booking not found" });
//       }
...
//           title: "Dispute Resolved",
//           message:
//             "Dispute reviewed and resolved. Provider will receive payment.",
//           category: "dispute",
//           bookingId: booking._id,
//           disputeId: dispute._id,
//         });
...
function getPaymentRangeRelevantDate(payment) {
  const status = toUpperStatus(payment?.status);
...
function filterPaymentsByRange(payments, range, from, to) {
  const bounds = getRangeBounds(range, from, to);
  if (!bounds) return payments;

  return payments.filter((payment) => {
    const relevantDate = getPaymentRangeRelevantDate(payment);
...
const isMockPaymentMode = () =>
  String(process.env.MOCK_PAYMENT_MODE || "").trim().toLowerCase() === "true";

async function finalizeSuccessfulPayment(payment, receipt = {}) {
  if (!payment) return null;

  if (payment.status === "FUNDS_HELD" || payment.status === "RELEASED") {
  ...
  wallet.pendingBalance += Number(payment.amount || 0);
  await wallet.save();
  ...
  try {
    ...
    console.error("Client notification failed:", notifError.message);
  }
  ...
  try {
    const providerNotification =
      payment.purpose === "additional_escrow"
        ...
        ? {
            ...
            category: "booking",
            bookingId: booking._id,
            metadata: { isEmergency: true, mockPayment: true },
            sendEmail: true,
            sendSMS: true,
          }
        : {
            ...
            category: "booking",
            bookingId: booking._id,
            metadata: { isEmergency: false, mockPayment: true },
            sendEmail: true,
            sendSMS: false,
          };
    ...
    console.error("Provider notification failed:", notifError.message);
  }
  ...
  async (req, res, next) => {
    try {
      ...
      const payment = await Payment.create({
        ...
        amount: expectedAmount,
        purpose: isAdditionalEscrow ? "additional_escrow" : "initial_escrow",
        gateway: isMockPaymentMode() ? "Mock-eSewa" : "eSewa",
        status: "INITIATED",
      });
      ...
      if (isMockPaymentMode()) {
        const finalizedBooking = await finalizeSuccessfulPayment(payment, {
          status: "COMPLETE",
        ...
        return res.json({
          ...
          amount: expectedAmount,
          purpose: isAdditionalEscrow ? "additional_escrow" : "initial_escrow",
          redirectUrl: `${frontendUrl}/payment/success?booking_id=${
            finalizedBooking?._id || bookingId
          }&transaction_uuid=${transaction_uuid}&mock=true`,
        });
      }
...
router.get("/esewa/success", async (req, res) => {
  try {
    ...
    if (payment.status === "FUNDS_HELD" || payment.status === "RELEASED") {
      console.log("ℹ️ Payment already processed, skipping duplicate success handling");
      return res.redirect(
        `${frontendUrl}/payment/success?booking_id=${payment.bookingId}&transaction_uuid=${transaction_uuid}`
      );
    }
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const rangedPayments = filterPaymentsByRange(payments, range, from, to);
      const total = rangedPayments.length;
      const pagedPayments = rangedPayments.slice((page - 1) * limit, page * limit);
  ...
  async (req, res, next) => {
    try {
      ...
      if (decisionType === "RELEASE_FULL") {
        ...
        payment.escrowReleasedAt = payment.escrowReleasedAt || payment.releasedAt;
        payment.clientConfirmedAt = payment.clientConfirmedAt || payment.releasedAt;
        dispute.adminDecision.type = "RELEASE_FULL";
        booking.status = "completed";
        booking.paymentStatus = "released";
        ...
        payment.refundedAt = new Date();
        if (releaseToProvider > 0) {
          payment.releasedAt = payment.releasedAt || payment.refundedAt;
          payment.escrowReleasedAt =
            payment.escrowReleasedAt || payment.releasedAt;
          payment.clientConfirmedAt =
            payment.clientConfirmedAt || payment.releasedAt;
        }
        dispute.adminDecision.type = "REFUND_PARTIAL";
        dispute.adminDecision.refundAmount = refundToClient;
        booking.status = "completed";
