...
export default function NotificationsPage() {
  ...
  function getNotificationIcon(type = "") {
    ...
    if (["payment_released", "payment_confirmed", "booking_completed", "booking_confirmed", "quote_approved", "verification_approved"].includes(type)) {
      return <HiCheckCircle {...iconProps} className={`${baseClasses} text-green-500`} />;
    }

    if (["payment_refunded", "booking_no_show", "booking_cancelled", "quote_rejected", "verification_rejected", "dispute_created", "warning"].includes(type)) {
      return <HiExclamationTriangle {...iconProps} className={`${baseClasses} text-red-500`} />;
    }

    if (["refund_request", "quote_requested", "quote_sent", "payment_pending", "dispute_opened"].includes(type)) {
      return <HiInformationCircle {...iconProps} className={`${baseClasses} text-amber-500`} />;
    }
