...
export default function Signup() {
  ...
  useEffect(() => {
    if (!window.google || !import.meta.env.VITE_GOOGLE_CLIENT_ID || !loginWithGoogle) {
      return;
    }
  ...
  function handleGoogleSignup() {
    ...
    try {
      ...
      console.error(err);
      const msg = "Google signup failed.";
      setError(msg);
      toast.error(msg);
      setGoogleLoading(false);
    }
  ...
  const inputBase = "w-full bg-transparent py-3 text-sm outline-none";
  const wrapperBase =
  ...
  const getWrapperClass = (field) =>
    `${wrapperBase} ${
      touched[field] && fieldErrors[field]
        ...
        : "border-slate-200 focus-within:border-blue-400 focus-within:ring-2 focus-within:ring-blue-200"
    }`;
