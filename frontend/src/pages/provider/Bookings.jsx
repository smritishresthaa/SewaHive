...
function getProviderStatusMessage(booking) {
  const normalized = normalizeStatusForTab(booking?.status);
  const total = Number(booking?.totalAmount || 0);

  if (normalized === "expired") {
    return "This booking request expired because it was not accepted before the scheduled time window passed.";
  }

  if (normalized === "rejected") {
    return "This booking was rejected. Any client payment linked to it should have been refunded.";
  }

  if (normalized === "no_show" || booking?.status === "no-show") {
    return `This booking was marked as no-show because the service was not started in time. Client refund processed for NPR ${total.toLocaleString()}.`;
  }
  ...
  if (booking?.paymentStatus === "refunded" && booking?.status === "resolved_refunded") {
    return "This booking was resolved with a refund outcome.";
  }
...
export default function ProviderBookings() {
  ...
  function formatLocationLabel(booking) {
    ...
    if (coords?.length === 2) {
      const lat = Number(coords[1]).toFixed(2);
      const lng = Number(coords[0]).toFixed(2);
      return `${lat}°N, ${lng}°E`;
    }
  ...
  function getNormalizedStatus(status) {
    return normalizeStatusForTab(status);
  }
  ...
  }, [filter, appliedRange, appliedCustomFrom, appliedCustomTo]);
  ...
  async function fetchBookings() {
    try {
      ...
      const params = {
        status: apiStatus,
        ...buildRangeParams(appliedRange, appliedCustomFrom, appliedCustomTo),
      };

      const res = await api.get("/bookings/provider-bookings", { params });
      const fetchedBookings = Array.isArray(res.data?.bookings)
        ? res.data.bookings
        : [];
      ...
      if (filter === "completed" || filter === "all") {
        const completedBookings = fetchedBookings.filter(
          (b) => getNormalizedStatus(b.status) === "completed"
        );
      ...
      console.log("Bookings endpoint not available yet:", err?.message || err);
      setBookings([]);
      setReviews({});
    } finally {
      setLoading(false);
  ...
  async function handleComplete(booking) {
    ...
    try {
      ...
      toast.success(
        res.data?.message ||
          "Job marked as complete! Awaiting client confirmation..."
      );
      await fetchBookings();
    } catch (err) {
      toast.error(
        err?.response?.data?.message || "Failed to mark job as complete"
      );
    } finally {
      setProcessing((prev) => ({ ...prev, [bookingId]: false }));
    }
  ...
  function toggleAdjustmentForm(booking) {
    ...
    const defaultProposed = Number(
      (currentApprovedTotal + estimatedExtra).toFixed(2)
    );

    setShowAdjustmentForm((prev) => ({
      ...prev,
      [bookingId]: !currentlyOpen,
    }));
  ...
  async function handleSubmitAdjustedQuote(booking) {
    ...
    try {
      ...
      const res = await api.post(
        `/bookings/${bookingId}/propose-adjusted-quote`,
        formData,
        {
  ...
  const getStatusBadge = (status) => {
    ...
    const styles = {
      ...
      resolved_refunded: "bg-slate-100 text-slate-700",
    ...
    return (
      <span
        className={`rounded-full px-3 py-1 text-xs font-semibold uppercase ${
          styles[normalized] || styles[status] || "bg-gray-100 text-gray-700"
        }`}
      >
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {bookings.map((booking) => {
              ...
              const emergencyFee = Number(booking.emergencyFee || 0);
              const hasCoordinates = booking.location?.coordinates?.length === 2;
              const statusMessage = getProviderStatusMessage(booking);
                ...
                >
                  <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mb-4 grid gap-3 text-sm text-gray-600 sm:grid-cols-2">
                        ...
                        {booking.type === "normal" &&
                          (booking.scheduledAt || booking.schedule?.date) && (() => {
                            const bookingDateTime = booking.scheduledAt
                              ? new Date(booking.scheduledAt)
                              : new Date(booking.schedule.date);

                            if (!booking.scheduledAt && booking.schedule?.slot) {
                              const [h, m] = booking.schedule.slot.split(":").map(Number);
                      ...
                      {hasCoordinates && (
                        <div className="mb-4">
                          ...
                          <p className="mt-1 break-all text-xs text-gray-500">
                            Coordinates:{" "}
                            {Number(booking.location.coordinates[1]).toFixed(6)},{" "}
                            {Number(booking.location.coordinates[0]).toFixed(6)}
                          </p>
                        </div>
                      )}
                      ...
                      {normalizedStatus === "completed" && reviews[booking._id] && (
                          ...
                          <div className="mb-2 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                            <div>
                              ...
                              <p className="mt-1 text-xs text-gray-600">
                                From:{" "}
                                {reviews[booking._id].clientId?.profile?.name ||
                                  "Client"}
                              </p>
                            </div>
                      ...
                      <div className="flex flex-wrap items-center gap-4 text-sm">
                        <span className="font-semibold text-gray-900">
                          Total: NPR {bookingTotal.toLocaleString()}
                        </span>
                        {emergencyFee > 0 && (
                          <span className="text-orange-600">
                            (includes NPR {emergencyFee.toLocaleString()} emergency fee)
                          </span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex w-full flex-col gap-2 xl:min-w-[220px] xl:w-auto">
                      ...
                      {(completionPendingStatuses.includes(normalizedStatus) ||
