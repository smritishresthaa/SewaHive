...
function getProviderSettlementSummary(booking, dispute) {
  ...
  if (paymentStatus === "partially_refunded") {
    return {
      title: "Partial dispute settlement",
      tone: "amber",
      lines: [
        `Client refund: NPR ${formatMoney(refundAmount)}.`,
        `Your payout: NPR ${formatMoney(providerPayout)}.`,
      ],
    };
  }
  ...
  if (paymentStatus === "refunded") {
    if (normalizedStatus === "no_show" || bookingStatus === "no-show") {
      return {
        title: "No payout due to no-show",
        tone: "red",
        lines: [
          `This booking was marked as no-show and the client was refunded NPR ${formatMoney(
            refundAmount || total
          )}.`,
        ],
      };
    ...
    if (normalizedStatus === "expired" || bookingStatus === "expired") {
      return {
        ...
        lines: [
          ...
          `Any client payment linked to this request was refunded.`,
        ],
      };
    }
    ...
    if (normalizedStatus === "rejected" || bookingStatus === "rejected") {
      return {
        title: "No payout after rejection",
        tone: "red",
        lines: [
          "The client payment was refunded because this booking was rejected.",
        ],
      };
    }
    ...
    if (resolution?.resolutionType === "refund_full") {
      return {
        title: "Full dispute refund",
        tone: "red",
        lines: [
          `Admin refunded NPR ${formatMoney(
            refundAmount || total
          )} to the client, so no payout was released.`,
  ...
  if (paymentStatus === "funds_held") {
    return {
      ...
      tone: "blue",
      lines: [
        "Payment is still held securely and will release after completion or resolution.",
      ],
    };
  }
...
export default function ProviderBookingDetail() {
  ...
  const settlementSummary = useMemo(
    () => getProviderSettlementSummary(booking, dispute),
    [booking, dispute]
  );
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <div className="rounded-2xl border bg-white p-5 shadow-sm sm:p-6">
            ...
            <div className="min-w-0 space-y-3">
              ...
              {booking.notes && (
                <div className="rounded-lg border border-gray-200 bg-gray-50 p-3">
                  <p className="text-sm font-medium text-gray-700">Client Requirements</p>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            {!isQuote && booking.status === "requested" && normalizedStatus !== "expired" && (
              <>
                <button
            ...
            {(normalizedStatus === "expired" ||
              ...
              <div className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-2 text-sm text-slate-700">
                No further action available for this booking.
              </div>
            )}
