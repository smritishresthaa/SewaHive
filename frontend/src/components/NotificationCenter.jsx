...
export default function NotificationCenter() {
  ...
  function getNotificationIcon(type = "") {
    if (["payment_refunded", "booking_no_show", "warning"].includes(type) || type.includes("rejected") || type.includes("cancelled")) {
      return <HiExclamationTriangle className="w-5 h-5 text-red-600" />;
    }
    if (["payment_released", "booking_confirmed", "booking_completed"].includes(type) || type.includes("approved") || type.includes("accepted")) {
      return <HiCheckCircle className="w-5 h-5 text-green-600" />;
    }
    return <HiBell className="w-5 h-5 text-blue-600" />;
  }
  ...
  function getNotificationColor(type = "") {
    if (["payment_refunded", "booking_no_show"].includes(type) || type.includes("rejected") || type.includes("cancelled")) return "bg-red-50";
    if (["payment_released", "booking_completed"].includes(type) || type.includes("approved") || type.includes("accepted")) return "bg-green-50";
    if (["refund_request", "payment_pending", "dispute_opened"].includes(type) || type.includes("warning") || type.includes("pending")) return "bg-yellow-50";
    return "bg-blue-50";
  }
